class Input {
	...
	beginWindow() {
		for (const control of Input.disabledControls)
			DisableControlAction(0, control, true)

		if (IsPedInAnyVehicle(PlayerPedId()))
			for (const control of Input.disabledControlsInVehicle)
				DisableControlAction(0, control, true)
	...
	isRectHovered(x, y, w, h) {
		return !(this.#state.mousePosX < x || this.#state.mousePosX > x + w || this.#state.mousePosY < y || this.#state.mousePosY > y + h)
	}
}
